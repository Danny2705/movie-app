...
const CheckboxDropdown = ({ options, onSelectionChange, title }) => {
  ...
  return (
    <div
      ref={dropdownRef}
      className={`${darkMode ? "bg-black" : "bg-white"} relative w-full`}
    >
      <div
      ...
      >
        <span className='max-w-full truncate'>{getSelectedGenresText()}</span>{" "}
        <span>
          {isOpen ? (
            <MdOutlineKeyboardArrowUp />
          ) : (
        ...
        >
          {options.map((option) => (
            <div
              ...
              } ${selectedOptions.includes(option.value) && "bg-slate-800"}`}
